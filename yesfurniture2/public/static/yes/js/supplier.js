    ...
    layui.use(['form', 'layer', 'table', 'jquery', 'element'], function () {
      ...
      // 点击添加客户
      table.on('toolbar(supplier_lst)', function (obj) {
        if (obj.event === 'add_supplier') {
          ...
          layer.open({
            ...
            shadeClose: true,
            area: ['70%', '70%'],
            content: $('#add_supplier_form'),
            success: function (layero, index) {
              $('.edit_supplier_form').hide();
              $('.add_supplier_form').show();
            }
          });
          $('.fromreset').trigger('click');
        }
      ...
      // 监听客户添加
      form.on('submit(add_supplier_form)', function (data) {
        var data = data.field;
        delete data.id;
        $.ajax({
          type: 'POST', //请求类型
          url: '/yes/supplier/addPost', //URL
          data: data, //传递的参数
          dataType: "json", //返回的数据类型
          success:function(){
            table.reload('idsupplier');
            layer.closeAll();
            layer.msg('添加成功'); 
          }
        });
      ...
      form.on('submit(contact_form_form)', function (data) {
        $.ajax({
          type: 'POST', //请求类型
          url: '/yes/supplier/addContact', //URL
      ...
      // 监听顶级菜单修改
      form.on('submit(edit_supplier_form)', function (data) {
        $.ajax({
          type: 'POST', //请求类型
          url: '/yes/supplier/editPost', //URL
          data: data.field, //传递的参数
          dataType: "json", //返回的数据类型
          success:function(){
            table.reload('idsupplier');
      ...
      //监听工具条
      table.on('tool(supplier_lst)', function (obj) {
        var data = obj.data;
        if (obj.event === 'del') { //删除生产单
          layer.confirm('确定删除该供应商么？', function (index) {
            obj.del();
            layer.close(index);
            $.ajax({
              type: 'POST',
              url: '/yes/supplier/delPost',
              data: {
                id: data.id
              },
              dataType: "json",
      ...
      form.on('submit(add_bank_form)', function (data) {
        ...
        data.bank_type = 'supplier';
        $.ajax({
          type: 'POST', //请求类型
          url: '/yes/supplier/bankAdd', //URL
          data: data, //传递的参数
          dataType: "json", //返回的数据类型
        });
      });
